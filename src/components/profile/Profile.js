						...
						<div className="profile">
							<div className="row">
								...
								<div className="profileData col-md-9 row">
			                    	...
			                    	{/* Main */}
									<div className="info col-md-8 col-sm-12 d-flex flex-column align-items-center justify-content-center">
										{ /* Edit icon */}
			                    		<div style={{display: ((this.state.showItem)? 'block' : 'none')}} onClick={this.displayProfileForm}>
											<span id="editBtn" className="fas fa-edit"></span>
										</div>
										{/* Profile data */}
										<div className="data" style={{display: ((this.state.showItem)? 'block' : 'none')}}>
											<h3 className="name">Nombre</h3>
											<div>Lorem ipsum dolor sit amet consectetur adipiscing elit torquent, dis ac netus tellus leo massa gravida, mattis suscipit ante purus malesuada pulvinar.
											</div>
										</div>
										{/* Form */}
										<form className="profileForm" style={{display: ((!this.state.showItem)? 'flex' : 'none')}}>
											<TextField name="nickname" label="Nickname" onChange={this.checkForm}/> {/* Value: nombre original (sera required) */}
											<TextField name="description" label="Description" rowsMax="2" multiline onChange={this.checkForm}/>
											<input type="file" name="profileImage"/>
											<div className="formButtons">
												<button type="reset" className="btn btn-secondary" onClick={this.displayProfileForm}>Cancel</button>
												<button type="submit" className="btn btn-success" onClick={this.checkForm}>Save</button>
											</div>
										</form>
									</div>
			                    </div>
